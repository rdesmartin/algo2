...
for iteration in range(0, N_iterations):
    ...
    title = f"voronoi : iteration  {iteration} (centroids in green)"
    plt.title(title)
    plt.xlabel('x')
    plt.ylabel('y')
    plt.savefig(f"algorithm/it_{iteration}_assign_voronoi.pdf")
    plt.close()
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.savefig(f"algorithm/it_{iteration}_move_centroids.pdf")
    plt.close()
